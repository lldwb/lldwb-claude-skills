...
`session-summary` 按一次会话总结运行的自迭代清单补两条纪律——「留下的状态」两面核验（现存性实测、归属时点对照时间线）与第 6 步复审实现路径（SendMessage 恢复原审查 agent）；`AGENTS.md` 版本分级判例补 v2.6.1 判例（同轮多处环节增补也按小版本）。

### 变更

- **`session-summary` 第 2 步「留下的状态」补两面核验纪律**：**现存性**以实测为准（`git status` / `ls` / `find` 现查，不凭时间线记录推断）；**动作的归属时点 / 轮次**以时间线记录的时间戳逐项对照（时间线是唯一时点证据源，错在漏对照、不在记录本身）。实证：该次总结运行的对抗性审查抓出 4 条 🟡，其中 3 条（过程产物声称现存实已清理、收尾动作时点错归后一轮、备份清单漏列一份）同源于凭会话时间线记录写状态而未实测
- **`session-summary` 第 6 步补复审实现路径与报告口径**：复审优先 SendMessage 恢复原审查 agent 复验（其上下文已含首轮审查证据），环境不支持恢复时派全新只读子代理对修复后的产出整体复验；复审沿用首轮分级报告格式、聚焦修复项逐条复验。实证：同次运行以 SendMessage 恢复原审查 agent 完成两轮复审，路径可用但此前技能未记载
- **`AGENTS.md` 版本分级判例段补 v2.6.1 判例**：同轮多处环节增补同「补一步流程 / 一处输出」口径，不因数量翻级（v2.6.1：session-summary 同轮补对抗性审查、自迭代收尾、无 id 缺省目标三处环节，agent 评估中版本、用户拍板发小版本）——判例此前仅入本机记忆，补齐权威源落点

### 说明

- 本次为既有技能纪律补强与仓库规则补判例，无新增技能、无脚本改动（`extract-session.py` 未触碰，自测基线无须重跑），仓库结构与技能数量（28）未变，按分级取**小版本**。
- `.claude-plugin/marketplace.json` 版本号 2.6.1 → 2.6.2

## [2.6.1] - 2026-09-22

`session-summary` 三项流程增强——新增对抗性审查环节、自迭代机制（移至流程收尾执行）、无会话 id 时目标缺省为当前会话；`extract-session` 修复时间线过滤与压缩摘要统计三处口径缺陷并首次建立自测基线；`feature-dev` 注意事项补方案确认处置两条（自主执行模式须用户显式声明）；`AGENTS.md` 按实证补版本分级判例与两条已知坑。

### 新增

- **`session-summary` 新增对抗性审查环节（第 6 步）**：总结、教训落点、优化点清单与回流内容在交用户确认 / 提交目标仓库之前，必须经**未参与产出的独立只读子代理**对抗性审查（默认怀疑结论夸大、证据断链、归属错位、回流夹带未实证内容），prompt 用 `references/review-agent.md`；审查结论由主代理**逐条复核证据**后采纳，无证据的不采纳；🔴 偏差修复后必须**复审**（复验由审查者做，不由产出者自证），🟡 随清单一并交用户；修复 → 复审最多 3 轮，超限停下显式交用户，不静默放行
- **`session-summary` 自迭代机制与收尾时序（流程 7 步扩为 8 步）**：session-summary 自身同在迭代范围内、不设特殊通道，但其迭代**只在流程最后执行**（新增第 8 步「自迭代收尾」）——自迭代的输入是**整个当前会话**（从取证到实施完成的全过程，含审查与实施环节暴露的摩擦），提前触发只能拿到不完整的输入，故第 3 步审视其它技能时**不**收集本技能问题；自迭代清单同样过对抗性审查（复用第 6 步子代理与模板）与用户确认后实施，改过 `scripts/extract-session.py` 的必须重跑自测基线；**一轮即止**——本轮自迭代实施中新暴露的摩擦留待下次会话总结，防「审视 → 实施 → 再审视」的无限递归
- **`session-summary` 无会话 id 时目标缺省为当前会话**：当前会话的记录文件 = `~/.claude/projects/` 下本项目路径**转义目录**（非字母数字字符转 `-`）里**最近修改**的 `.jsonl`（本会话逐回合追加、mtime 通常最新），定位到文件后把路径直接传给脚本（会话 id 与 `--project` 都不需要）；多会话并行时最近修改的可能不是本会话——取修改时间最近的几个候选，比对**文件尾部**是否含本次触发语与最近动作，拿不准时向用户要会话 id
- **`extract-session` 自测基线（`scripts/selftest.py`，18 用例）**：覆盖四种抽取模式、过滤规则（Caveat / 斜杠命令解包 / 压缩摘要过滤与保留）、定位转义与退出码，含历史回归用例；改过 `extract-session.py` 必跑——此前抽取脚本没有回归基线，历史上缺陷都靠实际使用才暴露，本版本的两处统计口径缺陷即由该基线用例先行暴露

### 变更

- **`feature-dev` 注意事项补方案确认处置两条**：「确认请求无应答」——发出确认请求后无人应答时，**可逆且完全落在需求范围内**的改动按推荐方案先实施（汇报首段声明方案未获用户确认、如有不符可指出后调整），删除、历史改写、依赖或接口变更等不可逆或影响面大的改动、以及超出需求范围的方案调整，停在方案阶段等待确认；「自主执行模式须显式声明」——只有用户明确声明进入自主执行模式才按上一条处置推进，**用户没说就代表不是自主执行模式**，不把「用户不在实时旁观、无法交互确认」等环境状态当作认定依据，未声明时一律按普通交互流程停在方案阶段等待确认
- **`AGENTS.md` 补三条实证规则**：版本分级补判例——单技能改名且正文流程结构不变、互指完整同步的按中版本处理（v2.6.0 判例），给既有技能补一步流程 / 一处输出的按小版本处理（v2.4.1 判例），替代原「如技能改名」笼统例子；已知坑 gitee 条目补失效令牌坑——`GITEE_TOKEN` 环境变量优先，本机残留的失效旧令牌被按**匿名**处理、私有仓库报 404 而非 401，跑脚本前先用令牌文件覆盖；新增多会话并行提交隔离坑——并行会话的暂存中间态会被全仓自检误报，提交一律显式路径限定并 `git show --stat` 复核归属

### 修复

- **`extract-session` 时间线模式斜杠命令用户消息被整条丢弃**：`mode_timeline` 以 `<` 前缀过滤用户消息，技能派发的会话（用户输入被 `<command-message>` 包装、`<command-args>` 才是用户原话）在时间线里用户输入全部不可见、上下文压缩摘要反被计入——消息预处理提为共用的 `clean_user_text()`（Caveat 过滤、命令包装解包、压缩摘要过滤），`mode_user` 与 `mode_timeline` 共用，`--user` 模式行为不变；实测两个被总结会话：用户消息 0→1、误计摘要 1→真实 3 条
- **`extract-session` 压缩摘要统计两处口径缺陷**：`--user` 模式默认过滤路径上摘要命中后未计数即跳过，「已过滤 N 条」提示对其服务的默认模式永远是 0；`--with-summary` 保留摘要时提示仍写「已过滤」、与实际保留自相矛盾——默认过滤路径前置计数，保留模式统计改为「含 N 条上下文压缩摘要」

### 说明

- 本次为既有技能流程补环节、脚本缺陷修复与仓库规则补强，无新增技能，仓库结构与技能数量（28）未变，按分级取**小版本**。
- `.claude-plugin/marketplace.json` 版本号 2.6.0 → 2.6.1

## [2.6.0] - 2026-09-22

把 `commit-review` 扩展为多对象代码评审并改名 `code-review`——评审对象从单一提交扩展为提交 / 合并 / 模块 / 项目整体 / 架构五种，结论口径按对象区分；同轮给 `skill-router` 汇报末尾补可复制的派发指令，并按实证会话给 `bug-fix` / `app-packaging` 各补取证与实测手法。

### 新增

- **`code-review`（原 `commit-review`）扩展为五种评审对象，SKILL.md 以模式路由表组织**：提交评审（单个修订号，二值结论）/ 合并评审（分支区间或 merge 提交，二值结论，作合入闸门）/ 模块评审（指定目录，分级报告）/ 项目整体评审（全仓横切面，分级报告 + 优先级路线）/ 架构评审（结构、分层与依赖设计，分级报告 + 演进建议）；通用六维清单对象无关、一处维护（SSOT 不复制），第七维提交信息规范仅提交/合并评审适用
- **三份对象专属清单**：`references/checklist-module.md`（模块内聚与职责、对外接口稳定、缺陷风险深读抽查 + 子代理分片指引）、`checklist-project.md`（横切一致性、技术债务分布、安全与敏感信息命中即严重 + 优先级路线口径）、`checklist-architecture.md`（分层清晰度、依赖方向与循环依赖、契约与扩展点设计，每条建议标影响面与改造代价量级）
- **`review-fetch.py`（原 `check-commit.py`）新增 `--range <base>...<head>` 区间取数**：merge-base 聚合 diff + 提交序列 + 文件清单落盘；校验三点格式（两点区间会把目标分支改动计入差异，显式报错）、与位置参数互斥、引用与 merge-base 可求；单修订号用法行为不变，仍只取数不判定
- **`review-checklist.md` 补「合并评审差异点」节**：一至六维按聚合 diff 核查、第七维替换为提交序列规范性（重复改动呈现等）、区间为空时先核实不得直接判无问题
- **`skill-router` 汇报末尾新增可复制的派发指令**：唯一推荐确定后，按目标技能 `## 输入` 小节格式把用户原话重写为结构化任务描述（已确认信息直接带入、缺失字段标 `<待补>`），危险动作技能的确认要求随指令带上；边界仍「只判断不代执行」，启动由用户粘贴指令

### 变更

- **全仓互指同步**：`AGENTS.md`、根 `README.md`、`PLUGIN_README.md`、`marketplace.json` 及 `git-history-rewrite` / `mr-create` / `log-diagnose` / `commit-create` / `session-summary` / `skill-router` 约 19 处「只评审提交质量用 commit-review」边界句更新为 `code-review` 五对象表述；两条分发路径（安装清单、插件 skills 数组）同步指向 `skills/code-review`
- **`bug-fix` 取证补两条手法**：对接外部产物 / 上游系统时，行为假设延伸到上游仓库取证拿裁决性证据（构建脚本 / 打包配置 / 协议文档），不凭猜测放宽或收紧校验；截图条款补正向方向——截图常暴露报告没覆盖的实际操作形态，逐张核对操作细节再下结论
- **`app-packaging` 更新链路实测补「伪装旧版使用者」做法**：产物已是最新版、手头没有旧版可跑时，临时把版本号改回上一版重新打包一份伪装旧版使用者的产物再跑端到端更新链路，跑完立即还原版本号

### 说明

- 本次为既有技能大改（`commit-review` 改名 `code-review` 并扩展五种评审对象）+ `skill-router` 能力增强 + `bug-fix` / `app-packaging` 实证手法补强；改名的破坏性影响限于调用方引用名（仓内互指已全部同步），仓库结构、分发路径与技能数量（28）未变，按分级取**中版本**。
- `.claude-plugin/marketplace.json` 版本号 2.5.2 → 2.6.0
