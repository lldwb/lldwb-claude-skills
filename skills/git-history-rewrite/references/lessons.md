...
- **优先 `--index-filter`**：直接改索引里的 blob（始终是仓库对象形态），不检出文件、绕开 `autocrlf`，只需改个别文件时更快也更稳（`references/index-filter.sh` 是可照抄的模板）。
- **替换模式不锚行尾**：`sed 's/^### Added/### 新增/'` 不带 `$`，两种行尾下都成立。
- **按字节替换、不解码**：脚本以二进制模式读写、按**字面量**换字节——不经解码/编码，既不引入行尾转换，也不因文件含非 UTF-8 字节而崩；替换片段本身不含换行时，还天然绕开 CRLF / LF 差异。替换词是纯文本时**不要用正则**：`.` `*` `[` 等元字符会误伤无关内容。
- **多分支全历史批量替换：`--tree-filter` + 独立幂等脚本**：替换逻辑写成独立脚本（对每个检出目录独立处理、重复执行结果一致），由 `--tree-filter` 逐提交调用、`--all` 覆盖全部分支——比内联长命令可控，也便于先 dry-run。
