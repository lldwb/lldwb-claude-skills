...
仓库指引初始化（`/init` 的等价实现）：为项目生成或更新 agent 指引文档——正文写入 `AGENTS.md`（唯一权威源），`CLAUDE.md` 只保留一段指向。
...
- 提交边界：落盘核验后先问用户，显式 `git add` 两份文档，不自动 push；用户追加的其他改动按 `commit-create` 的规则另起提交，不与指引文档混合
- 提交质量：标题与正文之间空一行（多行走消息文件 + `git commit -F`）、提交前脱敏自检、提交后复核 `%s`/`%b`；询问无应答时不擅自提交，保留改动待确认
