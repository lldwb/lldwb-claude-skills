...
发版校验与 Release 补齐（版本号按「架构」第 4 条分级选取；脚本只核对一致性与补齐呈现层，**不定级**、也不判定该不该发版）：
