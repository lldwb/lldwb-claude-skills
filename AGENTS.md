...
- **多会话并行操作本仓库时提交必须隔离**：另一会话可能已暂存自己的改动（含 `git mv` 改名这类尚未改完 frontmatter 的中间态），全仓自检（`check-skills.py`）会把这些中间态误报成问题——自检不过先 `git status` / `git diff` 辨明改动归属再下结论，别急着回改自己的文件；提交一律显式路径限定（本就禁止 `git add -A` / `git add .`），提交后用 `git show --stat` 复核只含自己的文件、并行会话的暂存原样保留（2026-09-22 实证：code-review 改造与 skill-router 优化两个会话并行进行时）。
