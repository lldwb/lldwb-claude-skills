...
`module-batch`（多模块并行改造）、`controller-check`（逐 Controller 追溯校验规则）等技能以 **Agent 工具 `subagent_type: general-purpose` + `run_in_background=true` + 轮询任务输出** 调度子代理；调度方只做编排、校验落盘、合并结果，**不替子代理做追溯/改造**。改这些技能时保持「调度与执行分离」。
